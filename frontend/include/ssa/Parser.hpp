...
class Parser {
 public:
  ...
  std::unique_ptr<VMInterfaceExprAST> parseExternExpr();
 ...
 private:
  ...
  std::pair<std::unique_ptr<PrototypeAST>,std::unique_ptr<ExprAST>> parseFunctionLikeExpr();
  ...
  std::unique_ptr<ActualParameter> parseActualParameterRule();
